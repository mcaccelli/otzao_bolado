...
function onSay(player, words)
	...
	for i = 1, #achievements do	
		TotalPoints = TotalPoints + achievements[i].points
		
		if player:getStorageValue(i+ACHIEVEMENTS_BASE) > 0 then
			text[#text+1] =  "OK-" .. achievements[i].name .. "\n"		
			ArchPoints =  ArchPoints + achievements[i].points			
		else
			text[#text+1] =  "----" .. achievements[i].name .. "\n"
		end					
	end
	player:showTextDialog(1949, table.concat(text), false)
	player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE,'You have the amount of ' .. ArchPoints .. ' achievements Points out of ' .. TotalPoints  ..' available...'  )  
